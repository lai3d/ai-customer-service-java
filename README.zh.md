...
Java 这几行来自[本仓库](docs/benchmark.md)，.NET 那一行来自[那个仓库](https://github.com/lai3d/ai-customer-service-dotnet/blob/main/docs/benchmark.md)。
**.NET 那一行是在 Docker Desktop 的 Linux 虚拟机里测的，不在宿主机上**——这台机器刻意没装 .NET
SDK——所以它带着标注进表，而不是被留在表外：同一块芯片、不同的操作系统、中间隔着一层虚拟化，
一次查询嵌入在那边要 5.8ms，Go 在宿主机上是 2ms。先拿它和自己打桩嵌入的那一轮比（1644 ms、
608 req/s），再拿它和上面几行比。
...
| .NET，线程池，在 Linux 虚拟机里 | 2227 ms | 449 req/s | 1843 ms | 40 |
...
模型不会这样，因为它把载体线程池按核数封了顶：**它赢在更笨，不是更聪明。** .NET 用同样的封顶
到达同样的 40 个线程，还在下面多找到一个开关：ONNX Runtime 给每次前向传播各开一个按核数大小的线程池，
十八个并发查询就把 73 个线程压到 18 个核上；改成单线程后它的 p50 降了 47%。
