...
in `.env`, later ones are issued per customer in the operations admin. A customer's web page
does not call this API by hand: one script tag embeds the [chat widget](docs/widget.md),
with a widget key bound to the page's origin. Omit `conversationId`
...
| [Channels: Telegram](docs/channels.md) | One bot per tenant on the same chat service as the widget; polling for one process, webhooks for replicas; why the reply is plain text and where the customer's identity stops |
| [Order connectors](docs/connectors.md) | A tenant's Shopify store behind the order tool: the seam, the mapping, three outcomes for the model, and what is deliberately not there (proving who owns the order) |
...
- [x] **14 · Operations admin** — the staff side, built by a second session ([record](docs/operations-admin.md)): staff login and accounts with bounded sessions, the ticket loop, the turn record and conversations as it shows them, answer feedback, knowledge editing with versioned publication and rollback, an overview with a definition next to every number, and its own React front end (`admin-ui/`); later per tenant, with platform staff above
...
- [x] **16 · Tenant knowledge ingestion** — a customer's own help pages and PDFs into draft knowledge entries through Spring AI's readers, re-import as replacement, fetched on the knowledge role behind an SSRF guard; publishing stays the admin's explicit step
- [x] **17 · Channel and connector** — an embeddable [web widget](docs/widget.md) on the customer's own page, with widget keys bound to origins; a [Shopify connector](docs/connectors.md) behind `OrderLookup`, per tenant, the token encrypted at rest, with a stand-in for a demo without a store
- [ ] **19 · Subscription panels** — an [Xboard connector](docs/connectors.md#xboard-a-subscription-panel-read-as-the-customer) for the V2board family: the customer's own subscription, read with the customer's own panel token forwarded by the widget, so ownership is proved by construction; tickets raised in the panel as the customer; the panel's knowledge articles as an import source; a [Telegram channel](docs/channels.md), one bot per tenant, polling or webhooks; the Telegram user identified through the panel's binding, so the subscription tool works there too *(built)*
- [x] **18 · Evaluation** — a [golden set](docs/evaluation.md) per tenant run through the real chat path and scored by rule, the deflection rate sampled from real conversations, both as gauges on the Quality row of the dashboard; the bundled set measured live, 41 of 43
