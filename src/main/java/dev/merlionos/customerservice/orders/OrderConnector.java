 ...
 */
/**
 * @param adminPath for an Xboard panel, the secret path segment its admin API lives under
 *                  ({@code /api/v2/<adminPath>/...}), needed with the admin token for what the
 *                  customer's own token cannot do
 */
public record OrderConnector(String tenantId, String kind, String shopDomain, String baseUrl, String accessToken, String apiVersion,
                             String adminPath, Instant configuredAt, String configuredBy) {
    ...
    public OrderConnector masked() {
        ...
        return new OrderConnector(tenantId, kind, shopDomain, baseUrl, masked, apiVersion, adminPath, configuredAt, configuredBy);
    }
}
