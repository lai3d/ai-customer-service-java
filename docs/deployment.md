...
| `DEFAULT_TENANT_API_KEY` | first deploy | none | The default tenant's first API key on `/api/v1/**` (`Authorization: Bearer <key>`), seeded at startup **only into an empty `tenant_api_key` table**; later keys are issued and revoked in the operations admin, so rotating one never brings this one back. Format `cs_` + 8 key-id characters + a secret. Unset, the public API answers `401` to everyone and startup logs a warning. Read by `all` and `chat` processes. See [ADR 002](adr/002-tenancy.md). |
