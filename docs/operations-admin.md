...
| Tenants in the admin (ADR 002 step 2's API, `/admin/api/tenants`): a Tenants page for admins that lists tenants, creates one, enables and disables it (never `default`), and issues and revokes API keys; an issued key is shown once with a copy button and dismissed by the admin, since the server keeps only its hash | `admin-ui/src/pages/Tenants.tsx` | [#NN](https://github.com/lai3d/ai-customer-service-java/pull/NN) |
