...
| Piece | Where | PR |
| --- | --- | --- |
| A forbidden tool on a golden case (#80's `forbidTool`, the injection probe's assertion): a second tool picker on the case form beside the expected one, the account tool listed in both, the case row saying `never calls ...`; the walk writes such a case and checks that naming one tool as both expected and forbidden is refused | `admin-ui/src/pages/Evaluation.tsx`, `admin-ui/e2e/admin.spec.ts` | [#81](https://github.com/lai3d/ai-customer-service-java/pull/81) |
| The UI walked in a browser, in CI: Playwright against the built UI image proxying to the built application image on Postgres, a stack of its own (`admin-ui/e2e/docker-compose.yml`, no fixed container names, only the UI published) brought up by `scripts/verify-admin-ui.sh`; seven serial tests that do the first day's work: a refused and an accepted sign-in, the bundled corpus listed, paged and searched, a tenant created and its key shown once, the tenant's own staff created and signed in to find the walls where they should be, a password changed, the pilot report on a tenant with no traffic, and the API a closed door once signed out; its own workflow, on pull requests that touch what it walks. The walk found the going-live checklist reading the tenant once and not again: it re-reads now when a key is issued or a connector or bot is saved | `admin-ui/e2e/`, `admin-ui/playwright.config.ts`, `scripts/verify-admin-ui.sh`, `.github/workflows/admin-ui-e2e.yml` | [#78](https://github.com/lai3d/ai-customer-service-java/pull/78) |
...
| Going live, on the tenant's page: what a pilot tenant needs before it can go live, read from what already exists rather than kept as a state: a live API key, the tenant's own staff, knowledge published, an evaluation run, and, optional, an order system and a Telegram bot; each with where it stands and a link to where it is done | `admin-ui/src/components/Onboarding.tsx` | [#77](https://github.com/lai3d/ai-customer-service-java/pull/77) |
| A pilot report (BUSINESS-PLAN.md, "证明它值钱"): `GET /admin/api/reports/pilot?tenant=&days=` and a Report page, one tenant and one window, the numbers a customer is shown with their definitions: conversations and turns, escalated and flagged, the deflection rate, the latest evaluation run's pass, retrieval and answer rates, tokens, the spend priced per model from `app.cost.prices` with an unpriced model named rather than costed at zero, and the cost per conversation; evaluation conversations count nowhere; "Copy as text" for the message to the customer | `admin/PilotReport`, `admin/AdminReportController`, `admin-ui/src/pages/Report.tsx` | [#75](https://github.com/lai3d/ai-customer-service-java/pull/75) |
| A tenant's Telegram bot on its page and on a tenant admin's Account page (`/admin/api/tenants/{id}/telegram`): connect a BotFather token in polling or webhook mode, the token checked against Telegram before it is stored and shown masked from then on, whether this process is polling and the webhook URL shown; Test asks Telegram for the bot's username; Remove stops it; the tenant's three steps beside it | `admin-ui/src/components/Telegram.tsx` | [#73](https://github.com/lai3d/ai-customer-service-java/pull/73) |
| A tenant's order system on its page (business-plan step 2's connector API, `/admin/api/tenants/{id}/order-connector`): connect a Shopify store with the custom app's token, shown masked from then on, or an Xboard panel by its origin with the tenant's admin token and admin path optional (the customer's own token is what the panel is read with; the two add tickets raised in the panel and its articles as an import source, the Knowledge page's "Import the panel's articles"); test it, the store answering with its name; disconnect; the store owner's steps to a token beside it; the same section on a tenant admin's Account page, since a tenant's own admin may configure it | `admin-ui/src/components/OrderConnector.tsx` | [#66](https://github.com/lai3d/ai-customer-service-java/pull/66) |
| An Evaluation page (business-plan step 3's API, `/admin/api/evaluation`): the deflection rate over a chosen window with its definition, the latest run's pass ratios; runs started by an admin with a note (one per tenant at a time, every case a paid model call), polled while running, each run's results per case with the three checks, the failures sentence, the answer given and a link to the rehearsal conversation; the golden set listed with what each case expects, and edited by admins with lists typed comma-separated | `admin-ui/src/pages/Evaluation.tsx`, `admin-ui/src/lists.ts` | [#64](https://github.com/lai3d/ai-customer-service-java/pull/64) |
| An evaluation run's conversations are rehearsals, not customers' (`conversation.kind`, `V19`): out of the conversation list, the ticket queue and every overview count, in every tenant; the list shows them on request (`?kind=evaluation`, a Kind filter on the page), and a conversation or ticket opened by id is still there for the run's record | `chat/TurnRecords`, `admin/AdminOverview`, `ticket/JdbcTicketWorkflow` | [#62](https://github.com/lai3d/ai-customer-service-java/pull/62) |
| Imports on the Knowledge page (ADR 002 step 4's API): a URL or a PDF into draft entries, the import rows polled every two seconds while one runs and the entries reloaded when it finishes; the entries table filtered by text and by source (typed or bundled, or one import's own) and paged, since one document is hundreds of chunks; first on the page, then on the server (`EntryFilter`, `EntryPage`, `GET /admin/api/knowledge/entries?text=&source=&page=&size=`, over the knowledge-admin seam as well), on the owner's ask, so a tenant with tens of thousands of chunks lists as fast as one with eighteen | `rag/api/EntryFilter`, `rag/JdbcKnowledgeAdmin`, `admin-ui/src/pages/Knowledge.tsx` | [#60](https://github.com/lai3d/ai-customer-service-java/pull/60) |
| Staff belong to a tenant (ADR 002 step 5): `staff_account.tenant_id`, null for platform staff, who are admins by the schema (`V17`); the tenant rides on the session principal; every admin list and detail is scoped to the caller's tenant, platform staff see every tenant and narrow with `?tenant=`; a row outside the caller's tenant is missing, a tenant named in `?tenant=` that is not the caller's is refused and recorded; tenants and keys are platform's; the last-enabled-admin rule counts per tenant and per platform; `/admin/api/me` and the login reply carry `{username, role, tenant}`; the pages show a tenant column and a picker to platform staff, the conversation views the customer's own id beside ours | `admin/StaffScope`, `admin/StaffPrincipal`, `admin/StaffAccounts`, every `admin/*Controller`, `admin-ui/src/components/TenantPicker.tsx` | [#56](https://github.com/lai3d/ai-customer-service-java/pull/56) |
...
| Tenants in the admin (ADR 002 step 2's API, `/admin/api/tenants`): a Tenants page for admins that lists tenants, creates one, enables and disables it (never `default`), and issues and revokes API keys, of either kind: a secret key for a server, or a widget key bound to the origins typed on the form, shown with the script tag the tenant pastes into its page; an issued key is shown once with a copy button and dismissed by the admin, since the server keeps only its hash | `admin-ui/src/pages/Tenants.tsx` | [#52](https://github.com/lai3d/ai-customer-service-java/pull/52) |
...
- A browser walk in CI: the UI was driven by hand against the built image and a running
  ...
  Built in the fourth round: `scripts/verify-admin-ui.sh` and the Admin UI walk workflow.
