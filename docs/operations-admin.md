...
| A tenant's Telegram bot on its page and on a tenant admin's Account page (`/admin/api/tenants/{id}/telegram`): connect a BotFather token in polling or webhook mode, the token checked against Telegram before it is stored and shown masked from then on, whether this process is polling and the webhook URL shown; Test asks Telegram for the bot's username; Remove stops it; the tenant's three steps beside it | `admin-ui/src/components/Telegram.tsx` | [#NN](https://github.com/lai3d/ai-customer-service-java/pull/NN) |
