...
| Imports on the Knowledge page (ADR 002 step 4's API): a URL or a PDF into draft entries, the import rows polled every two seconds while one runs and the entries reloaded when it finishes; the entries table filtered by text and by source (typed or bundled, or one import's own) and paged on the page, since one document is hundreds of chunks | `admin-ui/src/pages/Knowledge.tsx`, `admin-ui/src/knowledgeFilter.ts` | [#60](https://github.com/lai3d/ai-customer-service-java/pull/60) |
